...
test('Ręczne dodawanie fiszki', async ({ page }) => {
  ...
  for (const link of links) {
    ...
    if ((text && text.toLowerCase().includes('fisz')) || 
  ...
  if (!addButton) {
    ...
    for (const button of buttons) {
      ...
      if (text && (text.toLowerCase().includes('dodaj') || 
  ...
  if (formInputs.length >= 2) {
    ...
    for (const input of formInputs) {
      ...
      if ((placeholder && (placeholder.toLowerCase().includes('przód') || placeholder.toLowerCase().includes('front'))) ||
        ...
        frontInput = input;
      } else if ((placeholder && (placeholder.toLowerCase().includes('tył') || placeholder.toLowerCase().includes('back'))) ||
  ...
  if (frontInput && backInput) {
    ...
    for (const button of saveButtons) {
      const text = await button.textContent();
      if (text && (text.toLowerCase().includes('zapisz') || 
