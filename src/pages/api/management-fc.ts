...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    if (filters.source && !["ai-full", "ai-edited", "manual"].includes(filters.source)) {
      return new Response(
        JSON.stringify(
          createErrorResponse("validation_error", "Parametr source musi mieć wartość: ai-full, ai-edited lub manual")
        ),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!bodyData.front || !bodyData.back) {
      return new Response(JSON.stringify(createErrorResponse("validation_error", "Pola front i back są wymagane")), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify(createErrorResponse("validation_error", (error as Error).message)), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (user.id === TEST_E2E_USER_ID) {
      ...
      const newId = testE2EFlashcards.length > 0 ? Math.max(...testE2EFlashcards.map((f) => f.id)) + 1 : 1;
