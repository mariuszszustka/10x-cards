...
const AcceptFlashcardsSchema = z.object({
  flashcards: z.array(AcceptFlashcardSchema).min(1, "Lista fiszek nie może być pusta"),
});
...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    if (!result.success) {
      ...
      return new Response(
        JSON.stringify(createErrorResponse("validation_error", "Walidacja fiszek nie powiodła się", details)),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
