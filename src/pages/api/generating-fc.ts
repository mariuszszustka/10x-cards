...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!result.success) {
      ...
      return new Response(
        JSON.stringify(createErrorResponse("validation_error", "Nieprawidłowe dane wejściowe", details)),
    ...
    const source_text_hash = crypto.createHash("sha256").update(body.source_text).digest("hex");
    ...
    try {
      await enqueueFlashcardGeneration(generation.id, user.id, body.source_text, body.model);
      console.log(
        `Zadanie generowania fiszek dla generacji ID ${generation.id} dodane do kolejki (model: ${body.model})`
      );
    } catch (error) {
    ...
    return new Response(JSON.stringify(createErrorResponse("server_error", "Błąd wewnętrzny serwera")), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
