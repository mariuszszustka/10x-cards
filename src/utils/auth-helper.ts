...
const SUPABASE_KEY = (import.meta.env.SUPABASE_KEY ||
  ...
  "dummy-key-for-testing") as string;
...
export function getAdjustedSupabaseUrl(requestHost: string): string {
  ...
  if (requestHost && requestHost.includes("192.168.") && LOCAL_SUPABASE_URL.includes("127.0.0.1")) {
...
export function getCookieOptions(requestHost: string): CookieOptions {
  const isLocalhost =
    requestHost.includes("127.0.0.1") || requestHost.includes("localhost") || requestHost.includes("192.168.");
...
export function setSessionCookie(cookies: AstroCookies, requestHost: string, sessionData: any): void {
