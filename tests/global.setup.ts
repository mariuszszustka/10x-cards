...
async function cleanupTestData(supabase: any, userId: string) {
  ...
  try {
    ...
    // 1. review_history
    const { error: reviewHistoryError } = await supabase.from("review_history").delete().eq("user_id", userId);
    ...
    // 2. review_sessions
    const { error: reviewSessionsError } = await supabase.from("review_sessions").delete().eq("user_id", userId);
    ...
    const { error: progressError } = await supabase.from("flashcard_learning_progress").delete().eq("user_id", userId);
