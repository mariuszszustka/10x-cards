...
test("Diagnoza problemu z przekierowaniem na about:blank", async ({ page }) => {
  ...
  console.log(
    "Test naprawczy: Formularz widoczny?",
...
test.describe("Testy autoryzacji", () => {
  ...
  test("Logowanie z poprawnymi danymi", async ({ page, context }) => {
    ...
    await page
      .waitForSelector('[data-testid="auth-login-form"]', { timeout: 10000 })
      .catch(() => console.log("Nie znaleziono formularza logowania po selektorze testowym"));

    // Czekamy na atrybut gotowości formularza
    await page
      .waitForSelector('[data-test-login-form-ready="true"]', { timeout: 5000 })
  ...
  test("Rejestracja nowego użytkownika", async ({ page }) => {
    ...
    try {
      ...
      const userMenuVisible = await page
        .getByTestId(DASHBOARD.USER_MENU)
        .isVisible()
        .catch(() => false);
      const welcomeMessageVisible = await page
    ...
    try {
      ...
      if (page.url().includes("/dashboard")) {
        ...
        const loginButtonVisible = await page
          .getByTestId(AUTH_BUTTONS.LOGIN)
          .isVisible()
          .catch(() => false);
        const registerButtonVisible = await page
          .getByTestId(AUTH_BUTTONS.REGISTER)
          .isVisible()
          .catch(() => false);
        ...
        const hasAuthData = await page.evaluate(() => {
          return localStorage.getItem("userId") !== null || localStorage.getItem("authSession") !== null;
        });
